...
// Certifique-se de que todos os plugins necessários estão no Cargo.toml e são importados aqui
use tauri_plugin_store;
use tauri_plugin_window_state; // Certifique-se de importar o window_state
...
pub fn run() {
    ...
    tauri::Builder::default()
        // Inicialização de todos os plugins AQUI (uma vez cada)
        ...
        // Plugins de persistência e gerenciamento de estado
        .plugin(tauri_plugin_store::Builder::new().build()) // Única inicialização do plugin de store
        .plugin(tauri_plugin_window_state::Builder::default().build()) // Inicialização do plugin window_state
        // Plugin de instância única
